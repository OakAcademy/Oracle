--##############################################################
-- Hi dear friends, This file is about using 
-- Ampersand (&) subtition to get parameter
...
--------- EXAMPLES ----------------------------------------------
	...
	select name,surname, salary from employee order by &col
	-- when run ,sql developer will ask for col value, we will write name , you can write whatever column name you want

	-- using multiple ampersand
	select * from &table order by &sortcolumnby desc nulls last
	fetch next &limit rows only
	-- when run ,sql developer will ask for table, sortcolumnby and limit values, we will write employee as table 
	-- , salary as sortcolumnby , and 4 as limit.
	-- you can write whatever column or table name you want

	--using everywhere ampersand 
	select name, &colname from &table where &filtercolumn=&filtervalue 
	order by &sort
	offset &offsetval rows fetch first &limitsize rows only
	-- when run ,sql developer will ask for colname, table, filtercolumn, filtervalue,sort,offsetval  and limitsize value, 
	-- we will write surname as colname,
	-- employee as table 
	-- name as filter column 
	-- 'Ken' as filter value  ( if you write Ken without quotes you will get error, because it addresses datavalue not a column name or table name )
	-- name as sort 
	-- 0 (zero) as offsetval
	-- 10 as limitsize
