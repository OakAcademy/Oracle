--##############################################################
-- Hi dear friends, This file is about WITH statement 
...
--------- EXAMPLES ----------------------------------------------
	...
	--get departments and their patient counts using with statement

	with deppatients as ( select d.departmentid, count(p.patientid) countofpatients
							from doctors d join inspections i 
									on ( d.doctorid = i.doctorid ) 
								join patients p on ( i.patientid = p.patientid )
							group by d.departmentid )
	select dep.departmentname, dp.countofpatients
	from departments dep join deppatients dp 
								on ( dep.departmentid = dp.departmentid)
