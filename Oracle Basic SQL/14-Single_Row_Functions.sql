...
--------- EXAMPLES ----------------------------------------------
	...
	-- all case manipulation functions
	select lower(name) lowercase_name, upper(name) uppercase_name, initcap(name) initcap_name
	from employee

	--list all records name inlude r letter
	select * from employee where upper(name) like '%R%'

	-- list records, concatenate their name and surname, put their salary $ sign as postfix if salary is not null else put "not applicable"
