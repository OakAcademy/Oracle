--##############################################################
-- Hi dear friends, This file is about SET operators 
...
--------- EXAMPLES ----------------------------------------------
	...
	-- combine patient, doctor and employee tables with union 
	select name, 'Doctor' Job from doctors
	union
	select name, 'Patient' Job from patients
	union
	select name, 'Employee' Job from employee

	-- combine patient, doctor and employee tables with union and sort 
	select name, 'Doctor' Job from doctors
	union
	select name, 'Patient' Job from patients
	union
	...
	--to see , the difference between union and union all
		--1. UNION 
		select * from doctors
		union
		select * from doctors
	...
	-- to emphasize that first union block identifies column aliases
	select name, 'Doctor' "work tag" from doctors
	union
	select name, 'Patient' Job from patients
	union
	select name, 'Employee' Job from employee
	...
	--minus, subtract a resultset from the other 

		-- query1, includes all doctor records 
		select * from doctors

		-- query2, includes only doctors having departmentid =2 
		select * from doctors where departmentid = 2

		-- this query retrieves records that their departmentid is not 2 
		select * from doctors
		minus 
		select * from doctors where departmentid = 2
	...
	--intersect, get common records 

		--1 inspect table 
		select * from countryresource

		-- retrieve intersection set of natural resources of russia and usa 
		select resourcename from countryresources where country ='USA'
		intersect
		select resourcename from countryresources where country ='RUSSIA'
