...
-- group functions with SELECT statement on oracle database
...
-- list all department names of employee table 
select department  from employee

-- list all department names of employee table distinctive
select distinct department  from employee

--department counts 
select department,count(*) from employee group by department

--using distinct keyword in count() function
select department, count(distinct salary)  as distinct_count_of_salary,
count(*) as count_of_salary
...
-- using all main group functions, take care of avg_salary 
select department, min(salary) min_salary, max(salary) max_salary, avg(salary) avg_salary, sum(salary) sum_salary,
 count(* ) employee_count
 from employee group by department
 
-- using all main group functions
select department, min(salary) min_salary, max(salary) max_salary, avg(salary) avg_salary, sum(salary) sum_salary,
 count(* ) employee_count
 from employee group by department

 
--total query table basis  
select min(salary), max(salary),  avg(salary), sum(salary), count(*)  from employee

--to control 
select * from employee

--total query table basis, using nvl function in avg, when you have null values , these values effect your reporting queries 
select min(salary), max(salary),  avg(nvl(salary,0)), sum(salary), count(*)  from employee
